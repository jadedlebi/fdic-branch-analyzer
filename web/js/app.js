...
analysisForm.addEventListener('submit', async function(e) {
    ...
    const counties = formData.get('counties').trim();
...
function validateInput(input, type) {
    ...
    if (type === 'counties') {
        if (!value) {
            input.setCustomValidity('Please enter at least one county.');
        } else if (!value.includes(';') && !value.includes(',')) {
            // Single county - no validation needed
            input.setCustomValidity('');
        } else if (value.includes(';')) {
            // Multiple counties with semicolons
            const counties = value.split(';').map(c => c.trim()).filter(c => c);
            if (counties.length === 0) {
                input.setCustomValidity('Please enter valid county names.');
            } else {
                input.setCustomValidity('');
            }
        }
...
document.getElementById('counties').addEventListener('input', function() {
    validateInput(this, 'counties');
});
